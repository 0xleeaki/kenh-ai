---
title: Server Tools
description: Tools operated by kenh-ai that models can call during request
seoTitle: 'Server Tools | Model-Callable Tools Operated by kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/guides/features/server-tools
og:site_name: kenh-ai Documentation
og:title: 'Server Tools - Model-Callable Tools by kenh-ai'
og:description: 'Add powerful server-side tools like web search, web fetch, datetime, and image generation to any model on kenh-ai. Models decide when to call them — kenh-ai executes them automatically.'
og:image: https://kenh-ai.com/dynamic-og?title=Server%20Tools&description=Model-Callable%20Tools%20by%20kenh-ai
og:image:width: 1200
og:image:height: 630
...
Server tools are specialized tools operated by kenh-ai that any model can call during a request. When a model decides to use a server tool, kenh-ai executes it server-side and returns the result to the model — no client-side implementation needed.
...
| **Who executes it** | kenh-ai | kenh-ai | Your application |
...
**Server tools** are tools the model can invoke zero or more times during a request. kenh-ai handles execution transparently.
...
3. kenh-ai intercepts the tool call, executes it server-side, and returns the result to the model.
...
The model can call any combination of server tools and user-defined tools. kenh-ai executes the server tools automatically, while your application handles the user-defined tool calls as usual.
