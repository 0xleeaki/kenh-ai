...
canonicalUrl: https://kenh-ai.com/docs/guides/routing/routers/body-builder
og:site_name: kenh-ai Documentation
og:title: 'Body Builder - Generate Parallel API Requests'
og:description: "Use natural language to generate multiple kenh-ai API requests for parallel model execution."
og:image: https://kenh-ai.com/dynamic-og?title=Body%20Builder&description=Multi-model%20request%20generation
og:image:width: 1200
og:image:height: 630
...
The [Body Builder](https://kenh-ai.com/openrouter/bodybuilder) (`openrouter/bodybuilder`) transforms natural language prompts into structured kenh-ai API requests, enabling you to easily run the same task across multiple models in parallel.

## Overview

Body Builder uses AI to understand your intent and generate valid kenh-ai API request bodies. Simply describe what you want to accomplish and which models you want to use, and Body Builder returns ready-to-execute JSON requests.
...
Body Builder returns a JSON object containing an array of kenh-ai-compatible request bodies:
...
const builderResponse = await kenhAi.chat.send({
  model: 'openrouter/bodybuilder',
...
const results = await Promise.all(
  requests.map((req) => kenhAi.chat.send(req))
);
...
Body Builder has access to all available kenh-ai models and will:
...
<Warning>
Model slugs change as new versions are released. The examples below are current as of December 4, 2025. Check the [models page](https://kenh-ai.com/models) for the latest available models.
</Warning>
