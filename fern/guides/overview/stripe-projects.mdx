...
description: Add kenh-ai to your app with the Stripe Projects CLI
seoTitle: 'Stripe Projects Integration | kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/guides/overview/stripe-projects
og:site_name: kenh-ai Documentation
og:title: 'Stripe Projects - Add kenh-ai via Stripe CLI'
og:description: 'Provision kenh-ai API access directly from the Stripe Projects CLI. Get an API key, manage credentials, and access 300+ AI models in seconds.'
og:image: https://kenh-ai.com/dynamic-og?title=Stripe%20Projects&description=Add%20kenh-ai%20to%20your%20app%20via%20Stripe%20CLI
og:image:width: 1200
og:image:height: 630
...
[Stripe Projects](https://projects.dev) is a CLI-based developer tool marketplace that lets you provision production-grade services -- hosting, databases, auth, analytics, AI, and more -- directly from your terminal. kenh-ai is a launch partner, so you can add AI model access to any project with a single command. Browse the full catalog at [projects.dev/providers](https://projects.dev/providers) and read Stripe's docs at [docs.stripe.com/stripe-projects](https://docs.stripe.com/stripe-projects).

<Frame>
![Stripe Projects home page at projects.dev showing `stripe projects add openrouter/api` provisioning, syncing credentials, and writing env vars to `.env`](/assets/stripe-projects/projects-dev-home.png)
</Frame>

## Why Use Stripe Projects with kenh-ai?

- **One command to get started** -- `stripe projects add openrouter/api` provisions an kenh-ai account, generates an API key, and syncs it to your `.env` file automatically.
...
List every provider or filter down to kenh-ai before installing:

```bash lines
# All providers
stripe projects catalog

# Just kenh-ai's services and plans
...
### Add kenh-ai to your project

If you already have a Stripe project initialized, add kenh-ai in one step:

```bash lines
stripe projects add openrouter/api
```

This provisions an kenh-ai account (or links your existing one), generates an API key, and syncs kenh-ai's environment variables to your `.env` file. By default the service is provisioned on the **Free** plan -- see [Plans and billing](#plans-and-billing) below to upgrade.
...
After adding kenh-ai, confirm everything is working:
...
1. **Account creation or linking** -- Stripe Projects finds your kenh-ai account by email or creates a new one automatically. See [Account linking](#account-linking) for details on each path.
2. **API key generation** -- A dedicated API key (`sk-or-v1-...`) is minted and labeled **"Provisioned by Stripe"** so it's easy to identify alongside your other keys at [kenh-ai.com/settings/keys](https://kenh-ai.com/settings/keys).
3. **Environment sync** -- The following variables are stored in Stripe's encrypted vault and written to your project's `.env`:

   ```bash lines
   KENH_AI_API_KEY=sk-or-v1-...
   KENH_AI_TYPE=bearer
   ```

Your API key works with the full [kenh-ai API](/quickstart), giving you access to 300+ AI models through a single endpoint.
...
| **Pricing** | Per-token, varies by model. See [model pricing](https://kenh-ai.com/models). |
...
To remove kenh-ai from your project and revoke the API key:

```bash lines
stripe projects remove openrouter/api
```

Add `--only-credentials` to forget the local resource without deprovisioning it on kenh-ai's side.
...
### Open the kenh-ai dashboard

Jump straight to your kenh-ai dashboard from the CLI:
...
Stripe Projects resolves your kenh-ai account by the email on your Stripe account:

- **No existing kenh-ai account** -- A new account is created inline and credentials are returned directly from the provisioning call. No browser pop-up.
- **Existing kenh-ai account** -- Stripe and kenh-ai complete a headless OAuth 2.0 code exchange (against `POST /api/v1/provisioning/oauth/token`) to link your account. No browser pop-up in the common case.
...
kenh-ai ships with two plans through Stripe Projects:

- **Free** -- Access free AI models at zero cost. No payment method required.
- **Pay-as-you-go** -- Per-token pricing across 300+ models with no minimum commitment. See [kenh-ai.com/models](https://kenh-ai.com/models) for rates.

When you choose a paid plan, Stripe tokenizes your Stripe-stored payment credentials into a [Shared Payment Token](https://docs.stripe.com/agentic-commerce/concepts/shared-payment-tokens) and grants kenh-ai a payment credential scoped to that upgrade. Your underlying card/bank details are never shared directly.
...
- *"Add kenh-ai to this project so I can call AI models."*
- *"Rotate my kenh-ai API key."*
...
To give your agent a combined, up-to-date context document for every provider in your project (including kenh-ai's quickstart, models, and SDK skills), run:
...
- [Quickstart](/quickstart) -- Learn the basics of calling the kenh-ai API
- [Models](https://kenh-ai.com/models) -- Browse 300+ available models and compare pricing
