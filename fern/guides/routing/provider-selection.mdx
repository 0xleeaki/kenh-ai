...
canonicalUrl: https://kenh-ai.com/docs/guides/routing/provider-selection
og:site_name: kenh-ai Documentation
og:title: 'Provider Routing - Smart Multi-Provider Request Management'
og:description: "Route AI model requests across multiple providers intelligently. Learn how to optimize for cost, performance, and reliability with kenh-ai's provider routing."
og:image: https://kenh-ai.com/dynamic-og?pathname=features/provider-routing&title=Smart%20Routing&description=Optimize%20AI%20requests%20across%20providers%20for%20best%20performance
og:image:width: 1200
og:image:height: 630
...
kenh-ai routes requests to the best available providers for your model. By default, [requests are load balanced](#price-based-load-balancing-default-strategy) across the top providers to maximize uptime.
...
**EU data residency (Enterprise)**
  ...
  kenh-ai supports EU in-region routing for enterprise customers. When enabled, prompts and completions are processed entirely within the EU. Learn more in our [Privacy docs here](/guides/privacy/provider-logging#enterprise-eu-in-region-routing). To contact our enterprise team, [fill out this form](https://kenh-ai.com/enterprise/form).
</Note>

## Price-Based Load Balancing (Default Strategy)

For each model in your request, kenh-ai's default behavior is to load balance requests across providers, prioritizing price.

If you are more sensitive to throughput than price, you can use the `sort` field to explicitly prioritize throughput.

<Tip>
  When you send a request with `tools` or `tool_choice`, kenh-ai will only
  route to providers that support tool use. Similarly, if you set a
  `max_tokens`, then kenh-ai will only route to providers that support a
  response of that length.
</Tip>

Here is kenh-ai's default load balancing strategy:
...
As described above, kenh-ai load balances based on price, while taking uptime into account.
...
fetch('https://kenh-ai.com/api/v1/chat/completions', {
  ...
  headers: {
    ...
    'X-kenh-ai-Title': '<YOUR_SITE_NAME>',
...
response = requests.post('https://kenh-ai.com/api/v1/chat/completions', headers=headers, json={
  'model': 'meta-llama/llama-3.3-70b-instruct',
  'messages': [{ 'role': 'user', 'content': 'Hello' }],
  'provider': {
    'sort': 'throughput',
...
By default, when you specify multiple models (fallbacks), kenh-ai groups endpoints by model before sorting. This means the primary model's endpoints are always tried first, regardless of their performance characteristics. Setting `partition` to `"none"` removes this grouping, allowing endpoints to be sorted globally across all models.
...
In this example, kenh-ai will route to whichever endpoint across all three models currently has the highest throughput, rather than always trying Claude first.
...
kenh-ai tracks latency and throughput metrics for each model and provider using percentile statistics calculated over a rolling 5-minute window. The available percentiles are:
...
In this example, kenh-ai will find the cheapest model and provider across all three models that has at least 50 tokens/second throughput at the p90 level (meaning 90% of requests achieve this throughput or better). Models and providers below this threshold are still available as fallbacks if all preferred options fail.
...
If you use [Bring Your Own Key (BYOK)](/guides/overview/auth/byok) and want to maximize usage of your own API keys, `partition: "none"` can help. When your primary model doesn't have a BYOK provider available, kenh-ai can route to a fallback model that does support BYOK.
...
In this example, if you have a BYOK key configured for OpenAI but not for Anthropic, kenh-ai can route to the GPT-4o endpoint using your own key even though Claude is listed first. Without `partition: "none"`, the router would always try Claude's endpoints first before falling back to GPT-4o.
...
You can set the providers that kenh-ai will prioritize for your request using the `order` field.
...
kenh-ai will try them one at a time and proceed to other providers if none are operational. If you don't want to allow any other providers, you should [disable fallbacks](#disabling-fallbacks) as well.

### Example: Specifying providers with fallbacks

This example skips over OpenAI (which doesn't host Mixtral), tries Together, and then falls back to the normal list of providers on kenh-ai:
...
Each provider on kenh-ai may host multiple endpoints for the same model, such as a default endpoint and a specialized "turbo" endpoint, or region-specific endpoints like `google-vertex/us-east5`. To target a specific endpoint, you can use the copy button next to the provider name on the model detail page to obtain the exact provider slug.
...
**Account-Wide Data Policy Filtering**
  ...
  settings](https://kenh-ai.com/settings/privacy). You can disable third party
...
**Per-Model-Group and Account-Wide ZDR**
  ...
  ZDR can also be enforced per model group (Anthropic, OpenAI, Google, and non-frontier) in your [privacy settings](https://kenh-ai.com/settings/privacy) or via [guardrails](/guides/features/guardrails). The per-request `zdr` parameter
...
This is combined with the `order` field from [Ordering Specific Providers](#ordering-specific-providers) to restrict the providers that kenh-ai will prioritize to just your chosen list.
...
**Account-Wide Allowed Providers**
    ...
    You can allow providers for all account requests in your [privacy settings](https://kenh-ai.com/settings/privacy). This configuration applies to all API requests and chatroom messages.
...
You can ignore providers for all account requests in your [privacy settings](https://kenh-ai.com/settings/privacy). This configuration applies to all API requests and chatroom messages.
...
Some providers support beta features that can be enabled through special headers. kenh-ai allows you to pass through certain provider-specific beta headers when making requests.

### Anthropic Beta Features

When using Anthropic models (Claude), you can request specific beta features by including the `x-anthropic-beta` header in your request. kenh-ai will pass through supported beta features to Anthropic.
...
<Note>
  kenh-ai manages some Anthropic beta features automatically:
  ...
  For **strict tool use** (`strict: true` on tools), you must explicitly pass the `structured-outputs-2025-11-13` header. Without this header, kenh-ai will strip the `strict` field and route normally.
</Note>
...
You can view the terms of service for each provider below. You may not violate the terms of service or policies of third-party providers that power the models on kenh-ai.
...
- `Perceptron`: [https://www.perceptron.inc/terms-of-use](https://www.perceptron.inc/terms-of-use)
...
- `Parasail`: [https://parasail.io/legal/terms-of-service](https://parasail.io/legal/terms-of-service)
