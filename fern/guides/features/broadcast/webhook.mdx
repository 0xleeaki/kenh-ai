...
seoTitle: 'Broadcast to Webhook | kenh-ai Observability'
canonicalUrl: https://kenh-ai.com/docs/guides/features/broadcast/webhook
og:site_name: kenh-ai Documentation
og:title: 'Broadcast to Webhook - Send Traces to Any HTTP Endpoint'
og:description: 'Connect any HTTP endpoint to automatically receive traces from your kenh-ai requests. Step-by-step setup guide for Webhook integration.'
og:image: https://kenh-ai.com/dynamic-og?title=Webhook%20Broadcast&description=Send%20traces%20to%20any%20HTTP%20endpoint
og:image:width: 1200
og:image:height: 630
...
Click **Test Connection** to verify the setup. The configuration only saves if the test passes. During the test, kenh-ai sends an empty OTLP payload with an `X-Test-Connection: true` header to your endpoint.
...
Make an API request through kenh-ai and verify that your webhook endpoint receives the trace data.
