...
canonicalUrl: https://kenh-ai.com/docs/guides/features/server-tools/web-fetch
og:site_name: kenh-ai Documentation
og:title: 'Web Fetch Server Tool - URL Content Retrieval for Any Model'
og:description: 'Add URL content fetching to any model on kenh-ai. The model decides when to fetch, kenh-ai retrieves the content, and results are returned as text.'
og:image: https://kenh-ai.com/dynamic-og?title=Web%20Fetch%20Server%20Tool&description=URL%20content%20retrieval%20for%20any%20model
og:image:width: 1200
og:image:height: 630
...
document, it calls the tool with the URL. kenh-ai fetches and extracts the
...
3. kenh-ai fetches the URL using the configured engine (defaults to `auto`,
...
| Feature | Exa | Parallel | Firecrawl | kenh-ai | Native |
|---------|-----|----------|-----------|------------|--------|
...
1. Go to your [kenh-ai plugin settings](https://kenh-ai.com/settings/plugins)
   and configure your Firecrawl API key
2. Your Firecrawl account is billed separately from kenh-ai
...
- **kenh-ai/native engines**: Hard limit of 50 fetches per request
