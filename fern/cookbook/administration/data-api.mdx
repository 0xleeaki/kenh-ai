...
canonicalUrl: https://kenh-ai.com/docs/cookbook/administration/data-api
og:site_name: kenh-ai Documentation
og:title: 'Data API - Top 50 Daily Models by Token Usage'
og:description: 'Fetch the top 50 per-model token totals per day shown on kenh-ai.com/rankings as JSON, gated by your regular kenh-ai API key.'
og:image: https://kenh-ai.com/dynamic-og?title=Data%20API&description=Top%2050%20daily%20model%20token%20totals%20as%20JSON
og:image:width: 1200
og:image:height: 630
...
The Data API exposes the same dataset that powers the public [rankings page](https://kenh-ai.com/rankings) as JSON, so you can republish, analyse, or visualise it in your own tools.
...
The endpoint is gated by any valid kenh-ai API key. The same key you use to call `/api/v1/chat/completions` works here.
...
|---|---|---|
| `Authorization` | Yes | `Bearer <YOUR_KENH_AI_API_KEY>`. The same key you use to call `/api/v1/chat/completions` works here. |
...
The dataset updates live as traffic flows through kenh-ai — there is no daily refresh window — so a poll once per minute is more than enough to keep your view fresh. Exceeding either bucket returns `429 Too Many Requests`.
...
curl -G https://kenh-ai.com/api/v1/datasets/rankings-daily \
...
resp = requests.get(
    "https://kenh-ai.com/api/v1/datasets/rankings-daily",
    headers={"Authorization": f"Bearer {os.environ['KENH_AI_API_KEY']}"},
...
const res = await fetch(
  "https://kenh-ai.com/api/v1/datasets/rankings-daily?start_date=2026-04-01&end_date=2026-04-30",
  {
    headers: {
      Authorization: `Bearer ${process.env.KENH_AI_API_KEY}`,
    },
  },
);
if (!res.ok) {
...
When you republish, quote, or visualise data from this endpoint, kenh-ai must be cited using the canonical citation string below. The `{as_of}` value comes from `meta.as_of` in the response.
