...
description: How to transcribe audio into text with kenh-ai models
seoTitle: 'kenh-ai Speech-to-Text (STT) | Complete Documentation'
canonicalUrl: https://kenh-ai.com/docs/guides/overview/multimodal/stt
og:site_name: kenh-ai Documentation
og:title: 'kenh-ai Speech-to-Text - Complete Documentation'
og:description: 'Transcribe audio into text using AI models through the kenh-ai API. Access multiple STT providers with a single unified interface.'
og:image: https://kenh-ai.com/dynamic-og?title=kenh-ai%20Speech-to-Text&description=Transcribe%20audio%20into%20text%20using%20AI%20models%20through%20the%20kenh-ai%20API.
og:image:width: 1200
og:image:height: 630
...
kenh-ai supports speech-to-text (STT) via a dedicated `/api/v1/audio/transcriptions` endpoint. Send base64-encoded audio and receive a JSON response with the transcribed text and usage statistics.
...
curl "https://kenh-ai.com/api/v1/docs/guides/overview/models?output_modalities=transcription"
```
...
const result = await kenhAi.stt.createTranscription({
  model: '{{MODEL}}',
  inputAudio: {
    data: base64Audio,
    format: 'wav',
...
response = requests.post(
    url="https://kenh-ai.com/api/v1/audio/transcriptions",
    headers={
...
const response = await fetch('https://kenh-ai.com/api/v1/audio/transcriptions', {
  method: 'POST',
  headers: {
    Authorization: `Bearer {{API_KEY_REF}}`,
    'Content-Type': 'application/json',
...
curl https://kenh-ai.com/api/v1/audio/transcriptions \
...
```json lines
{
  ...
  "provider": {
    "options": {
      "groq": {
        "prompt": "Expected vocabulary: kenh-ai, API, transcription"
      }
    }
  }
}
...
STT supports [BYOK](/guides/overview/auth/byok), allowing you to use your own provider API keys. When configured, requests are routed directly to the provider using your key, and kenh-ai charges only its platform fee rather than the per-usage model cost.

## Playground

You can test STT models directly in the browser using the [kenh-ai Playground](https://kenh-ai.com/playground). Navigate to any STT model's page and use the playground tab to upload an audio file and see the transcription result.

## Differences from Audio Input

kenh-ai supports two ways to process audio:
...
- Ensure you're using a valid API key from [your kenh-ai dashboard](https://kenh-ai.com/settings/keys)
