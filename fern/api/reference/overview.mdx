---
title: API Reference
description: An overview of kenh-ai's API
seoTitle: 'kenh-ai API Reference | Complete API Documentation'
canonicalUrl: https://kenh-ai.com/docs/api/reference/overview
og:site_name: kenh-ai Documentation
og:title: 'kenh-ai API Reference - Complete Documentation'
og:description: "Comprehensive guide to kenh-ai's API. Learn about request/response schemas, authentication, parameters, and integration with multiple AI model providers."
og:image: https://kenh-ai.com/dynamic-og?title=kenh-ai%20API%20Reference&description=Comprehensive%20guide%20to%20kenh-ai's%20API.
og:image:width: 1200
og:image:height: 630
...
kenh-ai's request and response schemas are very similar to the OpenAI Chat API, with a few small differences. At a high level, **kenh-ai normalizes the schema across models and providers** so you only need to learn one.

## OpenAPI Specification

The complete kenh-ai API is documented using the OpenAPI specification. You can access the specification in either YAML or JSON format:

- **YAML**: [https://kenh-ai.com/openapi.yaml](https://kenh-ai.com/openapi.yaml)
- **JSON**: [https://kenh-ai.com/openapi.json](https://kenh-ai.com/openapi.json)
...
type Request = {
  ...
  // See "Plugins" section: kenh-ai.com/docs/guides/features/plugins
  plugins?: Plugin[];

  // See LLM Parameters (kenh-ai.com/docs/api/reference/parameters)
  ...
  // See models supporting tool calling: kenh-ai.com/models?supported_parameters=tools
  tools?: Tool[];
  ...
  // kenh-ai-only parameters
  // See "Model Routing" section: kenh-ai.com/docs/guides/features/model-routing
  models?: string[];
  route?: 'fallback';
  // See "Provider Routing" section: kenh-ai.com/docs/guides/routing/provider-selection
...
The `response_format` parameter allows you to enforce structured JSON responses from the model. kenh-ai supports two modes:
...
For detailed usage and examples, see [Structured Outputs](/guides/features/structured-outputs). To find models that support structured outputs, check the [models page](https://kenh-ai.com/models?supported_parameters=structured_outputs).

### Plugins

kenh-ai plugins extend model capabilities with features like web search, PDF processing, response healing, and context compression. Enable plugins by adding a `plugins` array to your request:
...
kenh-ai allows you to specify some optional headers to identify your app and make it discoverable to users on our site.

- `HTTP-Referer`: Identifies your app on kenh-ai.com
- `X-kenh-ai-Title`: Sets/modifies your app's title (`X-Title` also accepted)
- `X-kenh-ai-Categories`: Assigns marketplace categories (see [App Attribution](/app-attribution))
...
**Model routing**
  ...
  prefix. kenh-ai will select the least expensive and best GPUs available to
...
kenh-ai supports asking models to complete a partial response. This can be useful for guiding models to respond in a certain way.
...
kenh-ai normalizes the schema across models and providers to comply with the [OpenAI Chat API](https://platform.openai.com/docs/api-reference/chat).
...
// kenh-ai always returns detailed usage information.
...
kenh-ai normalizes each model's `finish_reason` to one of the following values: `tool_calls`, `stop`, `length`, `content_filter`, `error`.
...
const generation = await fetch(
  'https://kenh-ai.com/api/v1/generation?id=$GENERATION_ID',
  { headers },
);
