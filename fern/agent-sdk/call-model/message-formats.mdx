...
description: The kenh-ai SDK provides helper functions to convert between popular message formats. This makes it easy to migrate existing code or integrate with different APIs.
seoTitle: 'Message Formats | kenh-ai SDK'
canonicalUrl: 'https://kenh-ai.com/docs/agent-sdks/typescript/call-model/message-formats'
og:site_name: kenh-ai Documentation
og:title: 'Message Formats - kenh-ai SDK'
...
og:image: https://kenh-ai.com/dynamic-og?title=Message%20Formats&description=Format%20Conversion%20Helpers
og:image:width: 1200
og:image:height: 630
...
```typescript lines
import { kenh-ai, fromChatMessages } from '@kenh-ai/agent';
...
```typescript lines
import { toChatMessage } from '@kenh-ai/agent';
...
```typescript lines
import { kenh-ai, fromClaudeMessages } from '@kenh-ai/agent';
...
```typescript lines
import { toClaudeMessage } from '@kenh-ai/agent';
...
These features are Claude-specific and not supported by kenh-ai.
...
const openrouter = new KenhAI({ apiKey: process.env.KENH_AI_API_KEY });
const result = openrouter.callModel({
...
```typescript expandable lines
import { fromChatMessages, toChatMessage } from '@kenh-ai/agent';
