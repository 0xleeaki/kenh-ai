...
canonicalUrl: https://kenh-ai.com/docs/guides/routing/routers/latest-resolution
og:site_name: kenh-ai Documentation
og:title: 'Latest Model Resolution - Always Target the Newest Version'
og:description: "Use ~author/family-latest slugs to automatically route to the newest model in a family on kenh-ai."
og:image: https://kenh-ai.com/dynamic-og?title=Latest%20Model%20Resolution&description=Always%20target%20the%20newest%20version
og:image:width: 1200
og:image:height: 630
...
When a model author ships a new version (for example Anthropic releasing `claude-opus-4.8`), kenh-ai automatically starts routing `~anthropic/claude-opus-latest` to it. Older code calling the alias keeps working — it just runs on the newest version.
...
Each `~author/family-latest` slug is mapped to a model family on kenh-ai. When a request comes in:

1. **Slug recognition**: kenh-ai sees the `~` prefix and identifies which family the alias points to (for example `~anthropic/claude-opus-latest` → the Claude Opus family).
...
`~author/family-latest` rows on the [models page](https://kenh-ai.com/models) and in `/api/v1/models` responses report the pricing, context length, modalities, and supported parameters of the target they currently resolve to — not a frozen snapshot. This way:
