...
seoTitle: 'Add Human-in-the-Loop Controls to an Agent SDK Agent | kenh-ai Cookbook'
canonicalUrl: https://kenh-ai.com/docs/cookbook/building-agents/hitl-tools
og:site_name: kenh-ai Documentation
og:title: 'Add Human-in-the-Loop Controls to an Agent SDK Agent'
og:description: 'Add HITL controls to an existing kenh-ai Agent SDK agent so routine tool calls resolve automatically and high-stakes decisions pause for human review.'
og:image: https://kenh-ai.com/dynamic-og?title=Human-in-the-Loop%20Controls&description=Auto-resolve%20or%20pause%20for%20human%20review
og:image:width: 1200
og:image:height: 630
...
<Tip>
  This recipe assumes you already have an agent built with the kenh-ai Agent
...
- An existing TypeScript agent that uses `@kenh-ai/agent` and `callModel`
- An **kenh-ai API key** configured in that agent's environment
...
import { kenh-ai, tool } from '@kenh-ai/agent';
...
```typescript lines
// Keep using your existing kenh-ai client if your agent already has one.
