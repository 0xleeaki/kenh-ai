---
title: PDF Inputs
description: How to send PDFs to kenh-ai models
seoTitle: 'kenh-ai PDF Inputs | Complete Documentation'
canonicalUrl: https://kenh-ai.com/docs/guides/overview/multimodal/pdfs
og:site_name: kenh-ai Documentation
og:title: 'kenh-ai PDF Inputs - Complete Documentation'
og:description: 'Send PDF documents to any model on kenh-ai.'
og:image: https://kenh-ai.com/dynamic-og?title=kenh-ai%20PDF%20Inputs&description=Send%20PDF%20documents%20to%20any%20model%20on%20kenh-ai.
og:image:width: 1200
og:image:height: 630
...
kenh-ai supports PDF processing through the `/api/v1/chat/completions` API. PDFs can be sent as **direct URLs** or **base64-encoded data URLs** in the messages array, via the file content type. This feature works on **any** model on kenh-ai.
...
<Info>
  ...
  model. When the model does not support file input natively, kenh-ai will
...
To configure PDF processing, use the `plugins` parameter in your request. kenh-ai provides several PDF processing engines with different capabilities and pricing:
...
kenh-ai provides several PDF processing engines:
...
<Warning>
  OCR costs apply to all requests, including BYOK. kenh-ai uses its own Mistral
  key for OCR (not your BYOK key), so the per-page fee is always billed to your
  kenh-ai account.
</Warning>

If you don't explicitly specify an engine, kenh-ai will default first to the model's native file processing capabilities, and if that's not available, we will use the <code>"{DEFAULT_PDF_ENGINE}"</code> engine.

## OCR Image Limits

When the <code>"{PDFParserEngine.MistralOCR}"</code> engine extracts images from a PDF, kenh-ai requests at most **8 images per PDF** from Mistral via the OCR API's `image_limit` parameter, and forwards no more than 8 images per request to the downstream model. Surplus images are dropped while all extracted text is preserved in full.
...
const result = await kenhAi.chat.send({
  model: '{{MODEL}}',
  messages: [
    {
      role: 'user',
...
<Info>
  PDF URLs work with all processing engines. For Mistral OCR, the URL is passed directly to the service. For other engines, kenh-ai fetches the PDF and processes it internally.
</Info>
...
<Info>
  ...
  subsequent requests, kenh-ai will use this pre-parsed information instead
...
When kenh-ai parses a PDF, the response includes file annotations in the assistant message. Here is the TypeScript type for the annotation schema:
