...
canonicalUrl: https://kenh-ai.com/docs/guides/features/presets
og:site_name: kenh-ai Documentation
og:title: 'Presets - Configuration Management for AI Models'
og:description: "Learn how to use kenh-ai's presets to manage model configurations, system prompts, and parameters across your applications."
og:image: https://kenh-ai.com/dynamic-og?title=Presets&description=Configuration%20management%20for%20AI%20models
og:image:width: 1200
og:image:height: 630
...
[Presets](/guides/features/presets) allow you to separate your LLM configuration from your code. Create and manage presets through the kenh-ai web application to control provider routing, model selection, system prompts, and other parameters, then reference them in kenh-ai API requests.
...
1. [Create a preset](/guides/features/presets). For example, select a model and restrict provider routing to just a few providers.
    <Frame>
    ![Creating a new preset](/assets/preset-example.png "A new preset")
    </Frame>
...
you would send to the matching inference route — kenh-ai
...
```bash lines
curl https://kenh-ai.com/api/v1/presets/email-copywriter/chat/completions \
...
```bash lines
curl https://kenh-ai.com/api/v1/presets/code-reviewer/messages \
  -H "Authorization: Bearer $KENH_AI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash lines
curl https://kenh-ai.com/api/v1/presets/inbound-classifier/responses \
  -H "Authorization: Bearer $KENH_AI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "openai/gpt-4o",
