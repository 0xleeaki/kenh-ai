...
canonicalUrl: https://kenh-ai.com/docs/guides/features/plugins/web-search
og:site_name: kenh-ai Documentation
og:title: 'Web Search - Real-time Web Grounding for AI Models'
og:description: "Enable real-time web search capabilities in your AI model responses. Add factual, up-to-date information to any model's output with kenh-ai's web search feature."
og:image: https://kenh-ai.com/dynamic-og?pathname=features/web-search&title=Web%20Search&description=Add%20real-time%20web%20data%20to%20any%20AI%20model%20response
og:image:width: 1200
og:image:height: 630
...
You can incorporate relevant web search results for _any_ model on kenh-ai by activating and customizing the `web` plugin, or by appending `:online` to the model slug:
...
Web search results for all models (including native-only models like Perplexity and OpenAI Online) are available in the API and standardized by kenh-ai to follow the same annotation schema in the [OpenAI Chat Completion Message type](https://platform.openai.com/docs/api-reference/chat/object):
    ...
    "annotations": [
      {
        ...
        "url_citation": {
          ...
          "content": "Content of the web search result", // Added by kenh-ai if available
...
kenh-ai automatically adds the `x_search` tool
...
```json lines
{
  ...
  "messages": [
    {
      "role": "user",
      "content": "What are people saying about kenh-ai?"
    }
  ],
  "plugins": [{ "id": "web" }],
  "x_search_filter": {
...
Once set up, Firecrawl searches use your Firecrawl credits directly — there is no additional charge from kenh-ai.
...
[Parallel](https://parallel.ai) is a search engine that supports domain filtering and uses kenh-ai credits at \$0.005 per request. Includes up to 10 results in a request, then \$0.001 per additional result.
...
- **Exa search**: Uses kenh-ai credits at \$0.005 per request. Includes up to 10 results, then \$0.001 per additional result
- **Parallel search**: Uses kenh-ai credits at \$0.005 per request. Includes up to 10 results in a request, then \$0.001 per additional result
...
When using Exa search (either explicitly via `"engine": "exa"` or as fallback), the web plugin uses your kenh-ai credits and charges _\$0.005 per request_. This includes up to 10 results; additional results are charged at \$0.001 each, in addition to the LLM usage for the search result prompt tokens.
