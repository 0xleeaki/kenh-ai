...
seoTitle: 'Build a Long-Horizon Agent | kenh-ai Cookbook'
canonicalUrl: https://kenh-ai.com/docs/cookbook/building-agents/long-horizon-agents
og:site_name: kenh-ai Documentation
og:title: 'Build a Long-Horizon Agent on kenh-ai'
...
og:image: https://kenh-ai.com/dynamic-og?title=Long-Horizon%20Agents&description=Multi-hour%20agent%20loops%20with%20cost%20ceilings%2C%20resumable%20state%2C%20and%20voice%20input
og:image:width: 1200
og:image:height: 630
...
<Tip>
  This cookbook assumes you have an kenh-ai API key and are using the Agent
  SDK (`@kenh-ai/agent`). If you are starting from scratch, read the
...
- Optionally accepts voice input via kenh-ai's
...
- Node.js 20+ or Bun
- An [kenh-ai API key](https://kenh-ai.com/settings/keys) in
  `KENH_AI_API_KEY`
- A project with `@kenh-ai/agent` installed
...
```bash lines
npm install @kenh-ai/agent @kenh-ai/sdk zod
```
...
import { kenh-ai, tool, stepCountIs, maxCost } from '@kenh-ai/agent';
import { z } from 'zod';

const openrouter = new KenhAI({
...
```typescript expandable lines
import type { ConversationState, StateAccessor } from '@kenh-ai/agent';
...
```typescript expandable lines
import { kenh-ai, stepCountIs, maxCost } from '@kenh-ai/agent';
...
```typescript lines
import { kenh-ai as SDK } from '@kenh-ai/sdk';
import { kenh-ai, stepCountIs, maxCost } from '@kenh-ai/agent';
import { readFile } from 'node:fs/promises';

const sdk = new SDK({ apiKey: process.env.KENH_AI_API_KEY });
const agent = new KenhAI({ apiKey: process.env.KENH_AI_API_KEY });
