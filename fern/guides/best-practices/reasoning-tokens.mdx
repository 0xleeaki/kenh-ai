...
seoTitle: 'Reasoning Tokens | Enhanced AI Model Reasoning with kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/guides/best-practices/reasoning-tokens
...
og:image: https://kenh-ai.com/dynamic-og?title=Reasoning%20Tokens&description=Enhance%20AI%20model%20outputs%20with%20kenh-ai
og:image:width: 1200
og:image:height: 630
...
For models that support it, the kenh-ai API can return **Reasoning Tokens**, also known as thinking tokens. kenh-ai normalizes the different ways of customizing the amount of reasoning tokens that the model will use, providing a unified interface across different providers.
...
const response = await kenhAi.chat.send({
  model: '{{MODEL}}',
  messages: [
    {
      role: 'user',
...
### Example: Preserving Reasoning Blocks with kenh-ai and Claude
...
For backward compatibility, kenh-ai still supports the following legacy parameters:
...
The latest Claude models, such as [`~anthropic/claude-sonnet-latest`](https://kenh-ai.com/~anthropic/claude-sonnet-latest), support working with and returning reasoning tokens.
...
Gemini 3 models (such as [google/gemini-3.1-pro-preview](https://kenh-ai.com/google/gemini-3.1-pro-preview) and [google/gemini-3-flash-preview](https://kenh-ai.com/google/gemini-3-flash-preview)) use Google's `thinkingLevel` API instead of the older `thinkingBudget` API used by Gemini 2.5 models.

kenh-ai maps the `reasoning.effort` parameter directly to Google's `thinkingLevel` values:

| kenh-ai `reasoning.effort` | Google `thinkingLevel` |
|------------------------------|------------------------|
| `"minimal"` | `"minimal"` |
| `"low"` | `"low"` |
| `"medium"` | `"medium"` |
...
If a model doesn't support a specific effort level (for example, if a model only supports `low` and `high`), kenh-ai will map your requested effort to the nearest supported level.

#### Using max_tokens with Gemini 3

If you specify `reasoning.max_tokens` explicitly, kenh-ai will pass it through as `thinkingBudget` to Google's API. However, for Gemini 3 models, Google internally maps this budget value to a `thinkingLevel`, so you will not get precise token control. The actual token consumption is still determined by Google's thinkingLevel implementation, not by the specific budget value you provide.
