...
description: Use frame images to control the first or last frame of an kenh-ai video
seoTitle: 'Turn an Image into a Video | kenh-ai Cookbook'
canonicalUrl: https://kenh-ai.com/docs/cookbook/video-generation/image-to-video
og:site_name: kenh-ai Documentation
og:title: 'Turn an Image into a Video - kenh-ai Cookbook'
og:description: 'Submit an kenh-ai image-to-video job with frame_images, poll for completion, and download the finished clip.'
og:image: https://kenh-ai.com/dynamic-og?title=Turn%20an%20Image%20into%20a%20Video&description=Use%20frame%20images%20with%20kenh-ai%20video%20generation
og:image:width: 1200
og:image:height: 630
...
`frame_images` is for exact frame control. If you provide both `frame_images` and `input_references`, kenh-ai treats the request as image-to-video.
...
```bash lines
curl https://kenh-ai.com/api/v1/videos/models
```
...
async function waitForVideo(job) {
  ...
  for (let attempt = 1; attempt <= 60; attempt += 1) {
    ...
    const pollingUrl = new URL(current.polling_url, "https://kenh-ai.com");
...
async function downloadVideo(job) {
  const videoUrl =
    job.unsigned_urls?.[0] ??
    `https://kenh-ai.com/api/v1/videos/${job.id}/content?index=0`;

  const response = await fetch(videoUrl, {
    headers: videoUrl.startsWith("https://kenh-ai.com/api/")
      ? { Authorization: `Bearer ${apiKey}` }
      : undefined,
  });
