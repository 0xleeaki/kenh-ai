...
seoTitle: 'Generate and Download a Video from Text | kenh-ai Cookbook'
canonicalUrl: https://kenh-ai.com/docs/cookbook/video-generation/text-to-video
og:site_name: kenh-ai Documentation
og:title: 'Generate and Download a Video from Text - kenh-ai Cookbook'
og:description: 'Submit an kenh-ai text-to-video generation job, poll the job status, and download the finished video file.'
og:image: https://kenh-ai.com/dynamic-og?title=Generate%20and%20Download%20a%20Video%20from%20Text&description=Submit%2C%20poll%2C%20and%20save%20an%20kenh-ai%20video
og:image:width: 1200
og:image:height: 630
...
Use this guide when you need to add text-to-video generation to an app with kenh-ai.
...
const { data } = await fetch(
  "https://kenh-ai.com/api/v1/videos/models",
...
async function openrouter(path: string, init: RequestInit = {}) {
  const response = await fetch(`https://kenh-ai.com/api/v1${path}`, {
    ...init,
    headers: {
...
for (let attempt = 1; attempt <= 60; attempt += 1) {
  ...
  const pollingUrl = new URL(status.polling_url, "https://kenh-ai.com");
...
const downloadUrl =
  videoUrl ?? `https://kenh-ai.com/api/v1/videos/${job.id}/content?index=0`;

const videoResponse = await fetch(downloadUrl, {
  headers: downloadUrl.startsWith("https://kenh-ai.com/api/")
    ? { Authorization: `Bearer ${apiKey}` }
    : undefined,
});
