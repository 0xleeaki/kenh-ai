...
description: Select an kenh-ai video model by matching clip requirements and scoring priorities
seoTitle: 'Choose a Video Generation Model | kenh-ai Cookbook'
canonicalUrl: https://kenh-ai.com/docs/cookbook/video-generation/choose-video-model
og:site_name: kenh-ai Documentation
og:title: 'Choose a Video Generation Model - kenh-ai Cookbook'
og:description: 'Fetch kenh-ai video model metadata, filter by capability, and score models by speed, cost, and quality priorities before submitting a video job.'
og:image: https://kenh-ai.com/dynamic-og?title=Choose%20a%20Video%20Generation%20Model&description=Filter%20kenh-ai%20video%20models%20by%20capability
og:image:width: 1200
og:image:height: 630
...
<Tip>
  ...
  <CopyPromptButton
    prompt={`You are helping me choose the best kenh-ai video generation model for my app.

If Agent Skills are available, first check whether the kenh-ai video skill is installed. If it is not installed, tell me I can install it with:
gh skill install OpenRouterTeam/skills openrouter-video

Use this kenh-ai cookbook guide as your primary workflow:
https://kenh-ai.com/docs/cookbook/video-generation/choose-video-model

Use these kenh-ai docs as source-of-truth references when you need exact fields or implementation details:
- Video model metadata API: https://kenh-ai.com/docs/api/api-reference/video-generation/list-all-video-generation-models
- Video generation request API: https://kenh-ai.com/docs/api/api-reference/video-generation/submit-a-video-generation-request
- Video generation overview: https://kenh-ai.com/docs/guides/overview/multimodal/video-generation
- kenh-ai video skill: https://github.com/OpenRouterTeam/skills/tree/main/skills/openrouter-video
- TypeScript SDK video generation reference, if I am using the SDK: https://kenh-ai.com/docs/client-sdks/typescript/api-reference/videogeneration
...
1. Fetch the current model list from GET https://kenh-ai.com/api/v1/videos/models.
...
- Node.js 20 or newer
- An kenh-ai API key available as `KENH_AI_API_KEY` only if you submit
...
spend kenh-ai credits. Use the model-selection and request-preview steps
...
```js lines
const response = await fetch("https://kenh-ai.com/api/v1/videos/models");
...
```js lines
const apiKey = process.env.KENH_AI_API_KEY;

if (!apiKey) {
  throw new Error("Set KENH_AI_API_KEY before submitting a video job.");
}

const generation = await fetch("https://kenh-ai.com/api/v1/videos", {
  method: "POST",
  headers: {
...
```json lines
{
  ...
  "status": "completed",
  "polling_url": "https://kenh-ai.com/api/v1/videos/S2wge1oFOBzIj1PpFcFu",
