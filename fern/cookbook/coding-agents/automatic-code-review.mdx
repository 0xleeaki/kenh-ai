...
description: >-
  ...
  and kenh-ai — async, non-blocking reviews from a
  second model while you keep working
seoTitle: >-
  Automatic Code Review for Claude Code | kenh-ai
canonicalUrl: >-
  https://kenh-ai.com/docs/cookbook/coding-agents/automatic-code-review
og:site_name: kenh-ai Documentation
og:title: >-
  Automatic Code Review for Claude Code — Async
  Reviews via Hooks + kenh-ai
og:description: >-
  Set up automatic, non-blocking code reviews for
  Claude Code. A stop hook triggers a background
  reviewer agent via kenh-ai — observable,
  customizable, and cost-monitored.
og:image: >-
  https://kenh-ai.com/dynamic-og?title=Automatic%20Code%20Review&description=Async%20Reviews%20for%20Claude%20Code%20via%20Hooks%20%2B%20kenh-ai
og:image:width: 1200
og:image:height: 630
...
<Info>
  ...
  - Both agents are observable, customizable, and cost-monitored on [kenh-ai](https://kenh-ai.com)
...
Both agents route inference through kenh-ai, but
...
<Warning>
  The base URL is `https://kenh-ai.com/api` — **no
  `/v1` suffix**. This is kenh-ai's Anthropic Skin,
...
<Warning>
  ...
  - Codex uses `https://kenh-ai.com/api/v1` (with
    `/v1`), while Claude uses
    `https://kenh-ai.com/api` (without `/v1`) —
