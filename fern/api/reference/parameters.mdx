---
title: Parameters
seoTitle: 'API Parameters | Configure kenh-ai API Requests'
canonicalUrl: https://kenh-ai.com/docs/api/reference/parameters
og:site_name: kenh-ai Documentation
og:title: 'API Parameters - Complete Guide to Request Configuration'
og:description: 'Learn about all available parameters for kenh-ai API requests. Configure temperature, max tokens, top_p, and other model-specific settings.'
og:image: https://kenh-ai.com/dynamic-og?title=API%20Parameters&description=Complete%20guide%20to%20request%20configuration
og:image:width: 1200
og:image:height: 630
...
Sampling parameters shape the token generation process of the model. You may send any parameters from the following list, as well as others, to kenh-ai.

kenh-ai will default to the values listed below if certain parameters are absent from your request (for example, `temperature` to 1.0). We will also transmit some provider-specific parameters, such as `safe_prompt` for Mistral or `raw_mode` for Hyperbolic directly to the respective providers if specified.
