...
description: Send your first message and stream a response with the kenh-ai SDK
seoTitle: 'Quickstart: Build a Chat App | kenh-ai Cookbook'
canonicalUrl: https://kenh-ai.com/docs/cookbook/get-started/quickstart
og:site_name: kenh-ai Documentation
og:title: 'Quickstart: Build a Chat App with kenh-ai'
og:description: 'Build a working chat app in TypeScript that streams responses
from any model on kenh-ai. Install the SDK, send a message, and handle
multi-turn conversations.'
og:image: https://kenh-ai.com/dynamic-og?title=Quickstart%3A%20Build%20a%20Chat%20App&description=Stream%20responses%20from%20any%20model%20on%20kenh-ai
og:image:width: 1200
og:image:height: 630
...
**Goal:** Learn the fundamentals of kenh-ai by building a TypeScript chat
app that sends messages and streams responses through kenh-ai.
...
<Tip>
  ...
  <CopyPromptButton
    prompt={`Build a TypeScript command-line chat app that uses kenh-ai.

Goal:
Create a small Node.js app that sends a message to kenh-ai, streams the
assistant response, keeps multi-turn conversation context, and can switch
models by changing one string.

Build it as a way to teach me the basics of kenh-ai. Add short comments
...
- KENH_AI_API_KEY from the environment. Never hard-code the key.

Set up the project:

1. Create a project directory, for example openrouter-chat.
2. Run npm init -y.
3. Run npm pkg set type=module because @kenh-ai/sdk is ESM-only.
4. Install @kenh-ai/sdk.
5. Install tsx as a dev dependency.
6. Create chat.ts.
7. Run the app with KENH_AI_API_KEY set, for example:
   KENH_AI_API_KEY=sk-or-v1-... npx tsx chat.ts

If KENH_AI_API_KEY is missing, tell me to create a key at:
<https://kenh-ai.com/settings/keys>

Then help me add it to my local environment before running the app.

Use the current kenh-ai SDK request shape:

- Import the named client with:
  import { KenhAI } from '@kenh-ai/sdk';
- Do not use a default kenh-ai import.
- Create the client with:
  const client = new KenhAI({ apiKey: process.env.KENH_AI_API_KEY });
...
- The final explanation mentions the SDK's named kenh-ai import,
...
- An **kenh-ai API key** — create one at
  [kenh-ai.com/settings/keys](https://kenh-ai.com/settings/keys) or set up
  a new [Stripe project](/guides/overview/stripe-projects)

## 1. Create a project and install the SDK

Set up a new Node.js project and add the kenh-ai client SDK. The SDK is
...
kenh-ai gives you access to hundreds of models through one API. Change the
