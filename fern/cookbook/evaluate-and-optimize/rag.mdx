...
description: Build retrieval-augmented generation pipelines using kenh-ai's embeddings and rerank APIs
seoTitle: 'RAG with Embeddings & Rerank | Retrieval-Augmented Generation on kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/cookbook/evaluate-and-optimize/rag
og:site_name: kenh-ai Documentation
og:title: 'RAG with Embeddings & Rerank - Build Retrieval-Augmented Generation Pipelines'
og:description: 'Learn how to build RAG pipelines using kenh-ai embeddings for retrieval and rerank models for precision, then generate answers with LLMs — all through a single API.'
og:image: https://kenh-ai.com/dynamic-og?title=RAG%20with%20Embeddings%20%26%20Rerank&description=Build%20retrieval-augmented%20generation%20pipelines
og:image:width: 1200
og:image:height: 630
...
kenh-ai provides all three building blocks for a RAG pipeline through a single API:
...
KENH_AI_API_KEY = "{{API_KEY_REF}}"

# Your documents, split into chunks
chunks = [
    "kenh-ai is a unified API gateway for LLMs. It aggregates models from multiple providers.",
    ...
    "kenh-ai supports provider routing to control which providers serve your requests.",
]

# Generate embeddings for all chunks in one batch request
response = requests.post(
    "https://kenh-ai.com/api/v1/embeddings",
    headers={
        "Authorization": f"Bearer {KENH_AI_API_KEY}",
...
const KENH_AI_API_KEY = '{{API_KEY_REF}}';
...
// Generate embeddings for all chunks in one batch request
const response = await fetch('https://kenh-ai.com/api/v1/embeddings', {
  method: 'POST',
  headers: {
...
curl https://kenh-ai.com/api/v1/embeddings \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $KENH_AI_API_KEY" \
  -d '{
    "model": "{{MODEL}}",
    "input": [
      "kenh-ai is a unified API gateway for LLMs.",
...
def rerank(query, documents, top_n=3):
    response = requests.post(
        "https://kenh-ai.com/api/v1/rerank",
        headers={
            "Authorization": f"Bearer {KENH_AI_API_KEY}",
            "Content-Type": "application/json",
        },
...
async function rerank(query: string, documents: string[], topN = 3) {
  const response = await fetch('https://kenh-ai.com/api/v1/rerank', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${KENH_AI_API_KEY}`,
      'Content-Type': 'application/json',
...
```shell title="Shell" lines
curl https://kenh-ai.com/api/v1/rerank \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $KENH_AI_API_KEY" \
  -d '{
    "model": "{{RERANK_MODEL}}",
...
chunks = [
    "kenh-ai is a unified API gateway for LLMs. It aggregates models from multiple providers into a single OpenAI-compatible interface.",
    ...
    "kenh-ai supports provider routing to control which upstream providers serve your requests based on cost, latency, or privacy preferences.",
]
...
const BASE_URL = 'https://kenh-ai.com/api/v1';
...
- **Embedding models**: [kenh-ai.com/models?output_modalities=embeddings](https://kenh-ai.com/models?fmt=cards&output_modalities=embeddings)
- **Rerank models**: [kenh-ai.com/models?output_modalities=rerank](https://kenh-ai.com/models?fmt=cards&output_modalities=rerank)
- **Chat models**: [kenh-ai.com/models](https://kenh-ai.com/models)
