...
canonicalUrl: https://kenh-ai.com/docs/guides/routing/auto-exacto
...
og:image: https://kenh-ai.com/dynamic-og?title=Auto%20Exacto&description=Automatic%20tool-calling%20provider%20optimization
og:image:width: 1200
og:image:height: 630
...
- **Throughput** -- real-time tokens-per-second metrics (visible on the [Performance tab](https://kenh-ai.com/models) of any model page).
...
The tool-calling success rate signal is derived from the **Tool Call Error Rate** metric, which is also visible on the Performance tab of any model page. For each request that includes tools, kenh-ai inspects every tool call the model returned and validates it against the schemas the caller supplied.
...
`@cfworker/json-schema` delegates `pattern` and `patternProperties` to the runtime's built-in regex implementation. In kenh-ai's environment that is the native JavaScript `RegExp` (V8 / ECMA-262). There is no ECMA-262-conformance shim layered on top, so JavaScript regex semantics differ in some edge cases from the regex dialect specified by JSON Schema.
...
Without Auto Exacto, kenh-ai's default routing is primarily [price-weighted](/guides/routing/provider-selection#price-based-load-balancing-default-strategy) -- requests are load balanced across providers with a strong preference for lower cost. Auto Exacto changes this for tool-calling requests by reordering providers based on quality signals instead of price.
...
- **Default sort in account settings** -- set your default provider sort to price in your [account settings](https://kenh-ai.com/settings/preferences) to apply price sorting across all requests.
