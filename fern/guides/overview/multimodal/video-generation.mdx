...
description: How to generate videos with kenh-ai models
seoTitle: 'kenh-ai Video Generation | Complete Documentation'
canonicalUrl: https://kenh-ai.com/docs/guides/overview/multimodal/video-generation
og:site_name: kenh-ai Documentation
og:title: 'kenh-ai Video Generation - Complete Documentation'
og:description: 'Generate videos using AI models through the kenh-ai API.'
og:image: https://kenh-ai.com/dynamic-og?title=kenh-ai%20Video%20Generation&description=Generate%20videos%20using%20AI%20models%20through%20the%20kenh-ai%20API.
og:image:width: 1200
og:image:height: 630
...
kenh-ai supports video generation from text prompts (and optional reference images) via a dedicated asynchronous API. You can find the supported models, their capabilities, and pricing by filtering our [model list by video output](https://kenh-ai.com/docs/guides/overview/models?output_modalities=video).
...
```bash lines
curl "https://kenh-ai.com/api/v1/videos/docs/guides/overview/models"
```
...
curl "https://kenh-ai.com/api/v1/docs/guides/overview/models?output_modalities=video"
```
...
url = "https://kenh-ai.com/api/v1/videos"
headers = {
...
// Step 1: Submit the generation request
const response = await fetch('https://kenh-ai.com/api/v1/videos', {
  method: 'POST',
  headers,
  body: JSON.stringify({
    model: '{{MODEL}}',
...
# Step 1: Submit the generation request
curl -X POST "https://kenh-ai.com/api/v1/videos" \
...
# {
#   "id": "<job_id>",
#   "polling_url": "https://kenh-ai.com/api/v1/videos/<job_id>",
#   "status": "pending"
# }

# Step 2: Poll for status
curl "https://kenh-ai.com/api/v1/videos/<job_id>" \
  -H "Authorization: Bearer $KENH_AI_API_KEY"
...
{
  "id": "abc123",
  "polling_url": "https://kenh-ai.com/api/v1/videos/abc123",
  "status": "pending"
}
```
...
```json lines
{
  ...
  "status": "completed",
  "unsigned_urls": [
    "https://kenh-ai.com/api/v1/videos/abc123/content?index=0"
  ],
  "usage": {
    "cost": 0.25,
    "is_byok": false
...
```bash lines
curl "https://kenh-ai.com/api/v1/videos/{jobId}/content?index=0" \
  -H "Authorization: Bearer $KENH_AI_API_KEY" \
  --output video.mp4
```
...
`X-kenh-ai-Idempotency-Key` header of the form `<job_id>-<status>` for
...
You can configure a signing secret in your [workspace settings](/guides/features/workspaces) to verify that webhook payloads are authentically from kenh-ai. When a signing secret is configured, each webhook delivery includes an `X-kenh-ai-Signature` header.

The signature includes a timestamp and an HMAC hash:

```lines
X-kenh-ai-Signature: t=1234567890,v1=a1b2c3d4...
```
...
If you have ZDR enforcement enabled (either via [account settings](https://kenh-ai.com/settings/privacy) or the per-request `zdr` parameter), video generation requests will not be routed.
