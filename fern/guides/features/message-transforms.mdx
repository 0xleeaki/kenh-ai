...
seoTitle: 'Message Transforms | Pre-process AI Model Inputs with kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/guides/features/message-transforms
og:site_name: kenh-ai Documentation
og:title: 'Message Transforms - Optimize AI Model Inputs'
og:description: 'Transform and optimize messages before sending them to AI models. Learn about middle-out compression and context window optimization with kenh-ai.'
og:image: https://kenh-ai.com/dynamic-og?title=Message%20Transforms&description=Optimize%20AI%20model%20inputs%20with%20kenh-ai
og:image:width: 1200
og:image:height: 630
...
To help with prompts that exceed the maximum context size of a model, kenh-ai supports a context compression [plugin](/guides/features/plugins) that can be enabled per-request:
...
When context compression is enabled, kenh-ai will first try to find models whose context length is at least half of your total required tokens (input + completion). For example, if your prompt requires 10,000 tokens total, models with at least 5,000 context length will be considered. If no models meet this criteria, kenh-ai will fall back to using the model with the highest available context length.
...
<Note>
  [All kenh-ai endpoints](/guides/overview/models) with 8k (8,192 tokens) or less context
