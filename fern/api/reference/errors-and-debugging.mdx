...
seoTitle: 'API Error Handling and Debugging | kenh-ai Documentation'
canonicalUrl: https://kenh-ai.com/docs/api/reference/errors
og:site_name: kenh-ai Documentation
og:title: 'API Error Handling and Debugging - Complete Guide'
og:description: 'Learn how to handle errors and debug requests in kenh-ai API interactions. Comprehensive guide to error codes, messages, debugging options, and best practices.'
og:image: 'https://kenh-ai.com/dynamic-og?title=API%20Error%20Handling%20and%20Debugging&description=Learn%20how%20to%20handle%20errors%20and%20debug%20requests%20in%20kenh-ai%20API%20interactions.'
og:image:width: 1200
og:image:height: 630
...
For errors, kenh-ai returns a JSON response with the following shape:
...
```typescript lines
const request = await fetch('https://kenh-ai.com/...');
...
On <code>{HTTPStatus.S429_Too_Many_Requests}</code> and <code>{HTTPStatus.S503_Service_Unavailable}</code> responses, kenh-ai may include a standard HTTP `Retry-After` response header indicating how many seconds to wait before retrying.
...
The OpenAI SDK, Anthropic SDK, Vercel AI SDK, and kenh-ai SDK already respect this header for backoff. If you're using `fetch` directly, honor it before retrying:

```typescript lines
const res = await fetch('https://kenh-ai.com/api/v1/chat/completions', { ... });
...
When you opt in to [router metadata](/guides/features/router-metadata) via the `X-kenh-ai-Experimental-Metadata: enabled` header, the 403 response also includes the full `openrouter_metadata` object with routing context and a `pipeline` array detailing the guardrail stages that ran:
...
Different kenh-ai API endpoints handle errors in distinct ways:
...
kenh-ai provides a `debug` option that allows you to inspect the exact request body that was sent to the upstream provider. This is useful for understanding how kenh-ai transforms your request parameters to work with different providers.
...
When `debug.echo_upstream_body` is set to `true`, kenh-ai will send a debug chunk as the **first chunk** in the streaming response. This chunk will have an empty `choices` array and include a `debug` field containing the transformed request body:
...
1. **Understanding Parameter Transformations**: See how kenh-ai maps your parameters to provider-specific formats (e.g., how `max_tokens` is set, how `temperature` is handled).

2. **Verifying Message Formatting**: Check how kenh-ai combines and formats your messages for different providers (e.g., how system messages are concatenated, how user messages are merged).

3. **Checking Applied Defaults**: See what default values kenh-ai applies when parameters are not specified in your request.
...
kenh-ai will make a best effort to automatically redact potentially sensitive or noisy data from debug output. Remember that the debug option is not intended for production.
