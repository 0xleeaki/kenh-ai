...
seoTitle: 'Management API Keys | Programmatic Control of kenh-ai API Keys'
canonicalUrl: https://kenh-ai.com/docs/guides/overview/auth/management-api-keys
og:site_name: kenh-ai Documentation
og:title: 'Management API Keys - Programmatic Control of kenh-ai API Keys'
og:description: 'Manage kenh-ai API keys programmatically through dedicated management endpoints. Create, read, update, and delete API keys for automated key distribution and control.'
og:image: https://kenh-ai.com/dynamic-og?pathname=features/management-api-keys&title=Management%20API%20Keys&description=Programmatically%20manage%20kenh-ai%20API%20keys
og:image:width: 1200
og:image:height: 630
...
kenh-ai provides endpoints to programmatically manage your API keys, enabling key creation and management for applications that need to distribute or rotate keys automatically.

## Creating a Management API Key

To use the key management API, you first need to create a Management API key:

1. Go to the [Management API Keys page](https://kenh-ai.com/settings/management-keys)
2. Click "Create New Key"
3. Complete the key creation process

Management keys cannot be used to make API calls to kenh-ai's completion endpoints - they are exclusively for administrative operations.
...
const keys = await kenhAi.apiKeys.list();

// You can paginate using the offset parameter
const keysPage2 = await kenhAi.apiKeys.list({ offset: 100 });
...
const key = await kenhAi.apiKeys.get(keyHash);

// Update a key
const updatedKey = await kenhAi.apiKeys.update(keyHash, {
...
// Delete a key
await kenhAi.apiKeys.delete(keyHash);
```
