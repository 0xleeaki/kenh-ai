...
description: How to generate speech audio from text with kenh-ai models
seoTitle: 'kenh-ai Text-to-Speech (TTS) | Complete Documentation'
canonicalUrl: https://kenh-ai.com/docs/guides/overview/multimodal/tts
og:site_name: kenh-ai Documentation
og:title: 'kenh-ai Text-to-Speech - Complete Documentation'
og:description: 'Generate speech audio from text using AI models through the kenh-ai API.'
og:image: https://kenh-ai.com/dynamic-og?title=kenh-ai%20Text-to-Speech&description=Generate%20speech%20audio%20from%20text%20using%20AI%20models%20through%20the%20kenh-ai%20API.
og:image:width: 1200
og:image:height: 630
...
kenh-ai supports text-to-speech (TTS) via a dedicated `/api/v1/audio/speech` endpoint that is compatible with the [OpenAI Audio Speech API](https://platform.openai.com/docs/api-reference/audio/createSpeech). Send text and receive a raw audio byte stream in your chosen format.
...
curl "https://kenh-ai.com/api/v1/docs/guides/overview/docs/guides/overview/models?output_modalities=speech"
```
...
const stream = await kenhAi.tts.createSpeech({
  model: '{{MODEL}}',
...
response = requests.post(
  url="https://kenh-ai.com/api/v1/audio/speech",
  headers={
...
const response = await fetch('https://kenh-ai.com/api/v1/audio/speech', {
  method: 'POST',
  headers: {
...
curl https://kenh-ai.com/api/v1/audio/speech \
...
The TTS endpoint is fully compatible with the OpenAI SDK. You can use the OpenAI client libraries by pointing them at kenh-ai's base URL:
