---
title: "OpenClaw 🦞"
description: Use OpenClaw (formerly Moltbot, formerly Clawdbot) with kenh-ai
seoTitle: 'Integration with OpenClaw | kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/cookbook/coding-agents/openclaw-integration
og:site_name: kenh-ai Documentation
og:title: 'OpenClaw Integration - kenh-ai'
og:description: 'Learn how to configure OpenClaw to use kenh-ai for AI agents across multiple messaging platforms.'
og:image: https://kenh-ai.com/dynamic-og?title=OpenClaw&description=Use%20OpenClaw%20with%20kenh-ai
og:image:width: 1200
og:image:height: 630
...
The easiest way to configure OpenClaw with kenh-ai is using the built-in setup wizard:
...
1. Choosing kenh-ai as your provider
...
If you already have your kenh-ai API key and want to skip the wizard, use this one-line command:

```bash lines
openclaw onboard --auth-choice apiKey --token-provider openrouter --token "$KENH_AI_API_KEY"
```

This automatically configures OpenClaw to use kenh-ai with the recommended model (`openrouter/auto`).
...
Add your kenh-ai API key to your `~/.openclaw/openclaw.json`:
...
<Note>
That's it! OpenClaw has built-in support for kenh-ai. You don't need to configure `models.providers` - just set your API key and reference models with the `openrouter/<author>/<slug>` format.
</Note>
...
Your agents will now use kenh-ai to route requests to your chosen model.

## Model Format

OpenClaw uses the format `openrouter/<author>/<slug>` for kenh-ai models (prefix the author with `~` to track the latest version in a family). For example:
...
You can find the exact format for each model on the [kenh-ai models page](https://kenh-ai.com/models).
...
This provides an additional layer of reliability on top of kenh-ai's provider-level failover.
...
Learn more about how Auto Model works at [kenh-ai.com/models/openrouter/auto](https://kenh-ai.com/models/openrouter/auto).
...
```bash lines
openclaw auth set openrouter:default --key "$KENH_AI_API_KEY"
```
...
OpenClaw can't find your kenh-ai API key.

**Fix:**
1. Ensure the `KENH_AI_API_KEY` environment variable is set: `echo $KENH_AI_API_KEY`
2. Or verify your auth profile exists: `openclaw auth list`
3. Run the onboard command: `openclaw onboard --auth-choice apiKey --token-provider openrouter --token "$KENH_AI_API_KEY"`
...
**Fix:**
1. Verify the model ID is correct on the [kenh-ai models page](https://kenh-ai.com/models)
...
- [OpenClaw kenh-ai Provider Guide](https://docs.openclaw.ai/providers/openrouter)
