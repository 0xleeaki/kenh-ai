...
canonicalUrl: https://kenh-ai.com/docs/guides/features/server-tools/apply-patch
og:site_name: kenh-ai Documentation
og:title: 'Apply Patch Server Tool - File Editing for Coding Agents'
og:description: 'Enable any model on kenh-ai to propose file edits using V4A diff patches. The model generates the patch, kenh-ai validates it, and your application applies it.'
og:image: https://kenh-ai.com/dynamic-og?pathname=features/server-tools/apply-patch&title=Apply%20Patch%20Server%20Tool&description=File%20editing%20for%20coding%20agents
og:image:width: 1200
og:image:height: 630
...
3. kenh-ai validates the patch syntax (correct line prefixes, valid markers, non-empty paths).
...
This is a **human-in-the-loop** tool: kenh-ai validates the diff but never applies it. Your application is responsible for executing the file operation.
...
curl https://kenh-ai.com/api/v1/responses \
...
|-----------|------|---------|-------------|
| `engine` | string | `auto` | `auto` — uses native passthrough when the endpoint supports incremental diff streaming, otherwise falls back to kenh-ai's HITL validator. `native` — forces native passthrough (falls back to HITL if unsupported). `openrouter` — always uses the HITL validator, even on endpoints with native support. |
