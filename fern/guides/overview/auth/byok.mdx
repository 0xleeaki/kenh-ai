...
seoTitle: 'BYOK | Use Your Own Provider Keys with kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/guides/overview/auth/byok
og:site_name: kenh-ai Documentation
og:title: 'BYOK - Bring Your Own Keys to kenh-ai'
og:description: "Learn how to use your existing AI provider keys with kenh-ai. Integrate your own API keys while leveraging kenh-ai's unified interface and features."
og:image: https://kenh-ai.com/dynamic-og?title=BYOK&description=Bring%20Your%20Own%20Keys%20to%20kenh-ai
og:image:width: 1200
og:image:height: 630
...
kenh-ai supports both kenh-ai credits and the
option to bring your own provider keys (BYOK).

When you use kenh-ai credits, your rate limits for
each provider are managed by kenh-ai.
...
Manage keys in your [workspace BYOK settings](https://kenh-ai.com/workspaces/default/byok).

The cost of using custom provider keys on kenh-ai is
**{openRouterBYOKFee.fraction}%
of what the same model/provider would cost normally on
kenh-ai** and will be deducted from your kenh-ai
credits.
...
- **Prioritized** — Attempted in order, before falling
  back to kenh-ai endpoints. Use this section for your
  primary provider keys.
- **Fallback** — Tried only after kenh-ai endpoints
...
detail page (e.g.
[/workspaces/default/byok/openai](https://kenh-ai.com/workspaces/default/byok/openai)).

By default, if all keys in both sections encounter a
rate limit or failure, kenh-ai will fall back to
using shared kenh-ai endpoints.

You can toggle **"Always use for this provider"** on
individual prioritized keys to prevent any fallback to
kenh-ai endpoints. When enabled, kenh-ai will only
...
kenh-ai tries them in priority order (see
...
When you combine BYOK keys with [provider ordering](/guides/routing/provider-selection#ordering-specific-providers), kenh-ai **always prioritizes BYOK endpoints first**, regardless of where that provider appears in your specified order. After all BYOK endpoints are exhausted, kenh-ai falls back to shared capacity in the order you specified.
...
4. Amazon Bedrock (kenh-ai's shared capacity)
5. Google Vertex AI (kenh-ai's shared capacity)
6. Anthropic (kenh-ai's shared capacity)
...
2. Amazon Bedrock (kenh-ai's shared capacity)
3. Google Vertex AI (kenh-ai's shared capacity)
...
If you want to prevent fallback to kenh-ai endpoints
entirely, enable **"Always use for this provider"** on
your BYOK keys in your
[workspace BYOK settings](https://kenh-ai.com/workspaces/default/byok).
...
kenh-ai endpoints, then Fallback keys. You can
reorder keys via drag-and-drop on the provider detail
page (e.g.
[/workspaces/default/byok/openai](https://kenh-ai.com/workspaces/default/byok/openai)).
When a key fails (e.g. rate limit or error), kenh-ai
...
kenh-ai will try: First key, then Second key,
then kenh-ai endpoints, then Backup key.
...
- **API key filter** — Restrict which of your kenh-ai API keys can use this BYOK key. Useful for isolating BYOK usage to specific applications or environments.
...
4. **`model_slug`**: The kenh-ai model identifier you want to use this key for.
...
To use Amazon Bedrock with kenh-ai, you can authenticate using either Bedrock API keys or traditional AWS credentials.
...
For enhanced security, we recommend creating dedicated IAM users with limited permissions specifically for use with kenh-ai.
...
To use Google Vertex AI with kenh-ai, you'll need to provide your Google Cloud service account key in JSON format. The service account key should include all standard Google Cloud service account fields, with an optional `region` field for specifying the deployment region.
...
1. Navigate to your [Activity page](https://kenh-ai.com/activity) in the kenh-ai dashboard.
