...
seoTitle: 'Tool & Function Calling | Use Tools with kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/guides/features/tool-calling
og:site_name: kenh-ai Documentation
og:title: 'Tool & Function Calling - Use Tools with kenh-ai'
og:description: 'Use tools (or functions) in your prompts with kenh-ai. Learn how to use tools with OpenAI, Anthropic, and other models that support tool calling.'
og:image: https://kenh-ai.com/dynamic-og?title=Tool%20&%20Function%20Calling&description=Use%20tools%20with%20kenh-ai
og:image:width: 1200
og:image:height: 630
...
kenh-ai standardizes the tool calling interface across models and providers, making it easy to integrate external tools with any supported model.

**Supported Models**: You can find models that support tool calling by filtering on [kenh-ai.com/models?supported_parameters=tools](https://kenh-ai.com/models?supported_parameters=tools).

If you prefer to learn from a full end-to-end example, keep reading.

## Request Body Examples

Tool calling with kenh-ai involves three key steps. Here are the essential request body formats for each step:
...
Let's make the first kenh-ai API call to the model:
...
```typescript title="TypeScript (fetch)" lines
const request_1 = await fetch('https://kenh-ai.com/api/v1/chat/completions', {
  method: 'POST',
  headers: {
    Authorization: `Bearer {{API_KEY_REF}}`,
    'Content-Type': 'application/json',
...
Now, we can make a second kenh-ai API call, and hopefully get our result!
...
```typescript title="TypeScript SDK" lines
const response_2 = await kenhAi.chat.send({
  model: '{{MODEL}}',
  messages,
  tools,
  stream: false,
...
kenh-ai tracks how reliably each provider completes tool calls and surfaces this as the **Tool Call Error Rate** on the Performance tab of every model page. The same signal drives [Auto Exacto](/guides/routing/auto-exacto) provider ordering on tool-calling requests. For the exact validator, JSON Schema draft, regex semantics, and per-tool-call classification, see [How Tool-Calling Success Rate Is Measured](/guides/routing/auto-exacto#how-tool-calling-success-rate-is-measured).

For more details on kenh-ai's message format and tool parameters, see the [API Reference](/api/reference/overview).
