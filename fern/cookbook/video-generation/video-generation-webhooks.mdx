...
description: Submit a video job with a callback URL and verify kenh-ai webhook signatures
seoTitle: 'Get Video Results with Webhooks | kenh-ai Cookbook'
canonicalUrl: https://kenh-ai.com/docs/cookbook/video-generation/video-generation-webhooks
og:site_name: kenh-ai Documentation
og:title: 'Get Video Results with Webhooks - kenh-ai Cookbook'
og:description: 'Receive kenh-ai video job results through webhooks, preserve the raw request body, and verify the webhook signature.'
og:image: https://kenh-ai.com/dynamic-og?title=Get%20Video%20Results%20with%20Webhooks&description=Receive%20kenh-ai%20video%20job%20callbacks
og:image:width: 1200
og:image:height: 630
...
- An kenh-ai API key available as `KENH_AI_API_KEY`
- Node.js 20 or newer
- A public HTTPS endpoint for your webhook receiver
- A webhook signing secret configured in your kenh-ai workspace settings
...
const app = express();
const signingSecret = process.env.KENH_AI_WEBHOOK_SECRET;
...
app.post(
  ...
  (req, res) => {
    const signature = req.header("X-kenh-ai-Signature");

    if (!signature || !verifyOpenRouterSignature(req.body, signature)) {
      return res.sendStatus(401);
    }

    const idempotencyKey = req.header("X-kenh-ai-Idempotency-Key");
...
```bash lines
KENH_AI_WEBHOOK_SECRET=dev_secret npx tsx server.ts
```
...
kenh-ai can reach it over HTTPS.
...
const secret = process.env.KENH_AI_WEBHOOK_SECRET;

if (!secret) {
  throw new Error("Set KENH_AI_WEBHOOK_SECRET first.");
}
...
const response = await fetch("http://localhost:3000/openrouter/video-webhook", {
  ...
  headers: {
    ...
    "X-kenh-ai-Signature": `t=${timestamp},v1=${signature}`,
    "X-kenh-ai-Idempotency-Key": "job_test-completed",
  },
  body,
});
...
```bash lines
KENH_AI_WEBHOOK_SECRET=dev_secret node send-test-webhook.mjs
```
...
kenh-ai delivers the webhook and includes the expected headers and envelope.
...
if (!apiKey) {
  throw new Error("Set KENH_AI_API_KEY first.");
}
...
deduplicate retries with `X-kenh-ai-Idempotency-Key`, then download the
video from the first `unsigned_urls` entry or from the content endpoint. If the
URL points to the kenh-ai API, include the bearer token when downloading it.
...
Your receiver should return `204` for a valid kenh-ai webhook and `401` for
