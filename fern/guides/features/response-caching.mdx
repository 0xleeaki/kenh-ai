...
seoTitle: 'Response Caching | Cache API Responses with kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/guides/features/response-caching
og:site_name: kenh-ai Documentation
og:title: 'Response Caching - Cache Identical API Responses'
og:description: "Cache identical LLM requests with kenh-ai's response caching feature. Reduce costs and latency for agent workflows, unit testing, and repeated context processing."
og:image: https://kenh-ai.com/dynamic-og?title=Response%20Caching&description=Cache%20responses%20for%20identical%20API%20requests%20to%20save%20time%20and%20money
og:image:width: 1200
og:image:height: 630
...
Response caching allows you to cache responses for identical API requests. When a cached response is available, kenh-ai returns it immediately from cache with no billing (all billable usage counters are reported as `0`), reducing both latency and cost.

Response caching is **model-agnostic** and works with every model available on kenh-ai across all [supported endpoints](#supported-endpoints), regardless of provider. Caching operates at the kenh-ai layer before the request reaches any provider, so no provider-side support is required.
...
Add the `X-kenh-ai-Cache` header to enable caching for individual requests:
...
curl -i https://kenh-ai.com/api/v1/chat/completions \
  -H "Authorization: Bearer {{API_KEY_REF}}" \
  -H "Content-Type: application/json" \
  -H "X-kenh-ai-Cache: true" \
  -d '{
...
response = requests.post(
    "https://kenh-ai.com/api/v1/chat/completions",
    headers={
        "Authorization": f"Bearer {{API_KEY_REF}}",
        "Content-Type": "application/json",
        "X-kenh-ai-Cache": "true",
    },
    json={
        "model": "google/gemini-2.5-flash",
        "messages": [
...
const response = await fetch('https://kenh-ai.com/api/v1/chat/completions', {
  method: 'POST',
  headers: {
...
HTTP/2 200
X-kenh-ai-Cache-Status: MISS
X-kenh-ai-Cache-TTL: 300
```
...
HTTP/2 200
X-kenh-ai-Cache-Status: HIT
X-kenh-ai-Cache-Age: 12
X-kenh-ai-Cache-TTL: 288
...
When `cache_enabled` is set on a preset, caching is automatically applied to every request that references that preset. No `X-kenh-ai-Cache` header is required.
...
Two requests are considered identical when they share the same API key, model, endpoint type, streaming mode, and request body (including all parameters). When caching is enabled, kenh-ai generates a cache key from these inputs. If an identical request has been made before and the cached response has not expired, the cached response is returned immediately. Changing any of these–including the model, endpoint, or switching between streaming and non-streaming–produces a different cache key and a cache miss.

Since caching operates at the kenh-ai layer before the request is forwarded, it works with every model and provider across the [supported endpoint types](#supported-endpoints).
...
<Note>
**Non-determinism**: Cached responses are returned verbatim regardless of stochastic parameters like `temperature`. If you need fresh responses, use `X-kenh-ai-Cache-Clear: true` or a short TTL.
</Note>
...
2. `X-kenh-ai-Cache: false` header **disables** caching even if the preset enables it
3. `X-kenh-ai-Cache: true` **enables** caching when the preset does not configure caching (i.e. `cache_enabled` is absent)–but cannot override a preset that explicitly sets `cache_enabled: false` (rule 1 takes precedence)
4. `X-kenh-ai-Cache-TTL` header **overrides** the preset `cache_ttl_seconds` (default: 300 seconds)
...
|---|---|---|
| `X-kenh-ai-Cache` | `true` | Enable caching for this request |
| `X-kenh-ai-Cache` | `false` | Disable caching for this request (overrides preset) |
| `X-kenh-ai-Cache-TTL` | `<seconds>` | Custom TTL (1-86400 seconds, default 300) |
| `X-kenh-ai-Cache-Clear` | `true` | Force a cache refresh for this request |
...
| Header | Value | Description |
|---|---|---|
| `X-kenh-ai-Cache-Status` | `HIT` or `MISS` | Whether the response was served from cache |
| `X-kenh-ai-Cache-Age` | `<seconds>` | How long the response has been cached (on `HIT` only) |
| `X-kenh-ai-Cache-TTL` | `<seconds>` | Remaining TTL on `HIT`; full TTL on `MISS` |
...
You can customize the TTL per-request using the `X-kenh-ai-Cache-TTL` header, or set a default TTL in your [preset](/guides/features/presets) configuration.

## Cache Clearing

To force a fresh response for a specific request, send the `X-kenh-ai-Cache-Clear: true` header alongside `X-kenh-ai-Cache: true` (or with a preset that has `cache_enabled: true`). This deletes the existing cached entry for that cache key, makes a new request to the provider, and stores the new response. `X-kenh-ai-Cache-Clear` has no effect unless caching is enabled for the request. This does not clear all cached entries–only the one matching the current request.

The new cache entry uses the TTL from the current request's `X-kenh-ai-Cache-TTL` header, the preset `cache_ttl_seconds`, or the default (300 seconds), following the standard [precedence rules](#precedence).
