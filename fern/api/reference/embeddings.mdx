...
seoTitle: 'Embeddings API | Convert Text and Images to Vector Representations with kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/api/reference/embeddings
og:site_name: kenh-ai Documentation
og:title: 'Embeddings API - Generate Vector Embeddings from Text and Images'
og:description: "Generate vector embeddings from text and images using kenh-ai's unified embeddings API. Access multiple embedding models from different providers with a single interface."
og:image: https://kenh-ai.com/dynamic-og?title=Embeddings%20API&description=Generate%20Vector%20Embeddings%20from%20Text%20and%20Images
og:image:width: 1200
og:image:height: 630
...
Embeddings are numerical representations of text that capture semantic meaning. They convert text into vectors (arrays of numbers) that can be used for various machine learning tasks. kenh-ai provides a unified API to access embedding models from multiple providers.
...
const response = await kenhAi.embeddings.generate({
  model: '{{MODEL}}',
...
kenh-ai provides access to various embedding models from different providers. You can view all available embedding models at:

[https://kenh-ai.com/models?fmt=cards&output_modalities=embeddings](https://kenh-ai.com/models?fmt=cards&output_modalities=embeddings)
...
const models = await kenhAi.embeddings.listModels();
...
response = requests.get(
  "https://kenh-ai.com/api/v1/embeddings/models",
  headers={
    "Authorization": f"Bearer {{API_KEY_REF}}",
  }
)
...
```typescript title="TypeScript (fetch)" lines
const response = await fetch('https://kenh-ai.com/api/v1/embeddings/models', {
  headers: {
    'Authorization': 'Bearer {{API_KEY_REF}}',
  },
});
...
```shell title="Shell" lines
curl https://kenh-ai.com/api/v1/embeddings/models \
  -H "Authorization: Bearer $KENH_AI_API_KEY"
```
...
**402 Payment Required**: Insufficient credits. Add credits to your kenh-ai account.
...
- [Models Page](https://kenh-ai.com/models?fmt=cards&output_modalities=embeddings) - Browse all available embedding models
