...
description: Securely rotate your kenh-ai API keys
seoTitle: 'API Key Rotation | Secure Key Management for kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/cookbook/administration/api-key-rotation
og:site_name: kenh-ai Documentation
og:title: 'API Key Rotation - Secure Key Management'
og:description: 'Learn how to securely rotate your kenh-ai API keys using the key minting API. Includes best practices and BYOK advantages for simplified key management.'
og:image: https://kenh-ai.com/dynamic-og?title=API%20Key%20Rotation&description=Secure%20Key%20Management
og:image:width: 1200
og:image:height: 630
...
Regular API key rotation is a security best practice that limits the impact of compromised credentials. kenh-ai's [Management API](/guides/overview/auth/management-api-keys) makes it easy to rotate keys programmatically without service interruption.
...
First, you'll need a [Management API key](https://kenh-ai.com/settings/management-keys) to manage your API keys programmatically.
...
const newKey = await kenhAi.apiKeys.create({
...
BASE_URL = "https://kenh-ai.com/api/v1/keys"
...
const BASE_URL = 'https://kenh-ai.com/api/v1/keys';
...
await kenhAi.apiKeys.delete(oldKeyHash);
...
If you use [Bring Your Own Key (BYOK)](/guides/overview/auth/byok) with kenh-ai, you get a significant advantage when it comes to key rotation: **you can rotate your kenh-ai API keys without ever needing to rotate your provider keys**.

When you configure BYOK, your provider API keys (OpenAI, Anthropic, Google, etc.) are stored securely in kenh-ai and associated with your account, not with individual kenh-ai API keys. This means:

- **Rotate kenh-ai keys freely**: You can rotate your kenh-ai API keys as often as you like for security compliance without touching your provider credentials.
...
- **Single point of management**: Manage all your provider keys in one place through kenh-ai's [integrations settings](https://kenh-ai.com/settings/integrations), while rotating your application-facing kenh-ai keys independently.
...
<Tip>
With BYOK, your provider keys are tied to your kenh-ai account, not to individual API keys. Rotate your kenh-ai keys as often as needed without any changes to your provider configuration.
</Tip>
...
- **Monitor key usage**: Check the [Activity page](https://kenh-ai.com/activity) to verify traffic has migrated to the new key before deleting the old one.
