...
seoTitle: 'Broadcast to OpenTelemetry Collector | kenh-ai Observability'
canonicalUrl: https://kenh-ai.com/docs/guides/features/broadcast/otel-collector
og:site_name: kenh-ai Documentation
og:title: 'Broadcast to OpenTelemetry Collector - Send Traces to OTLP Backends'
og:description: 'Connect any OpenTelemetry-compatible backend to automatically receive traces from your kenh-ai requests. Step-by-step setup guide for OTLP integration.'
og:image: https://kenh-ai.com/dynamic-og?title=OpenTelemetry%20Collector%20Broadcast&description=Send%20traces%20to%20OTLP%20backends
og:image:width: 1200
og:image:height: 630
...
[OpenTelemetry](https://opentelemetry.io/) is an open-source observability framework for collecting, processing, and exporting telemetry data. kenh-ai can send traces to any backend that supports the OpenTelemetry Protocol (OTLP), including Axiom, Jaeger, Grafana Tempo, and self-hosted collectors.
...
Make an API request through kenh-ai and view the trace in your OpenTelemetry backend.
...
<Tip>
kenh-ai sends traces using the OTLP/HTTP protocol with JSON encoding. Ensure your collector or backend is configured to accept OTLP over HTTP on the `/v1/traces` path.
</Tip>
...
- Using `parent_span_id` lets you link kenh-ai traces to your application's existing distributed traces
