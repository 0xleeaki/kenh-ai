...
canonicalUrl: https://kenh-ai.com/docs/guides/routing/routers/pareto-router
og:site_name: kenh-ai Documentation
og:title: 'Pareto Router - Coding-Score-Based Model Selection'
og:description: "Route coding requests by a single minimum coding score using kenh-ai's Pareto Router."
og:image: https://kenh-ai.com/dynamic-og?title=Pareto%20Router&description=Coding-score-based%20model%20selection
og:image:width: 1200
og:image:height: 630
...
The [Pareto Router](https://kenh-ai.com/openrouter/pareto-code) (`openrouter/pareto-code`) is a way to have kenh-ai always pick a strong coding model for your needs without committing to a specific one. You express a single `min_coding_score` preference between `0` and `1`, and the router routes your request to a coding model that meets that bar.

## Overview

The Pareto Router is tuned for coding use cases. It maintains a curated shortlist of strong coding models currently available on kenh-ai, ranked by their [Artificial Analysis](https://artificialanalysis.ai/) coding percentile (an integer between `0` and `100` that captures how a model ranks within AA's benchmarked coding field). Your `min_coding_score` picks the tier of models you want to route to. Within the chosen tier the router selects the cheapest model that is currently available (or the fastest, when you request the `:nitro` variant).
...
1. Navigate to [Settings > Plugins](https://kenh-ai.com/settings/plugins)
...
<Info>
The router resolves a primary coding model plus up to two same-tier fallbacks. The primary is what serves your request. The fallbacks only fire on transient provider errors or rate limits, they do not load-balance traffic. If the entire tier has no models currently published on kenh-ai, the router steps into a neighboring tier instead. The response `model` field always reports the concrete model that handled the request.
</Info>
...
2. **Candidate filtering**: The router takes the tier's curated shortlist and filters it to models that are currently published on kenh-ai.
