...
seoTitle: 'Router Metadata | kenh-ai'
canonicalUrl: https://kenh-ai.com/docs/features/router-metadata
og:site_name: kenh-ai Documentation
og:title: 'Router Metadata - Inspect Routing Decisions on Every Response'
og:description: 'Opt in to kenh-ai routing metadata to see which provider was selected, fallback attempts, and pipeline stages applied to your request.'
og:image: https://kenh-ai.com/dynamic-og?title=Router%20Metadata&description=Inspect%20Routing%20Decisions%20on%20Every%20Response
og:image:width: 1200
og:image:height: 630
...
kenh-ai's router runs every request through a multi-stage pipeline: it picks a provider, may compress context, may run guardrails, may invoke server-side tools, and may retry against fallbacks. By default, none of that is visible on the response.
...
Opt in by sending the `X-kenh-ai-Experimental-Metadata` request header with the value `enabled`:
...
curl https://kenh-ai.com/api/v1/chat/completions \
  ...
  -H "X-kenh-ai-Experimental-Metadata: enabled" \
  -d '{
...
const response = await fetch('https://kenh-ai.com/api/v1/chat/completions', {
  ...
  headers: {
    ...
    'X-kenh-ai-Experimental-Metadata': 'enabled',
  },
  body: JSON.stringify({
    model: 'openai/gpt-4o-mini',
    messages: [{ role: 'user', content: 'Hello' }],
...
The `X-kenh-ai-Experimental-Metadata` opt-in header is the supported way to enable the feature, but the header name and accepted values may also change while the feature is experimental.
